...
#    binary can abort the whole shell inside the failed start (claudebar#6),
#    before any QML signal fires. sh always starts; a failed exec is sh
#    exiting 126/127, which the empty branch maps to the failed-start path.
check "the command is wrapped in sh" \
    grep -qF "proc.command = [\"/bin/sh\", \"-c\", 'exec \"\$0\" \"\$@\"', resolvedBin].concat(args)" "$panel"
check "proc.command is assigned exactly once" \
    test "$(grep -cF 'proc.command =' "$panel")" = 1
check_absent "the direct (unwrapped) form is banned" 'proc.command = [resolvedBin]' "$panel"
...
check "installCmd literal appears exactly once" \
    test "$(grep -cF 'yay -S codexbar' "$panel")" = 1
